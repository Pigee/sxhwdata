...
async def proc_json(request):
    ...
    sql = "call proc_t(%s)"
    cur_liupanshan.execute(sql,(request.json["name"]))
    all_obj = cur_liupanshan.fetchone()

    sql = "select GroupNo WorkerNo,count(1) OutsideCallTotalCnt from tbcallcdr where calltime >= %s and calltime <= %s and length(CallerNo) > 4 and GroupNo = %s"
    ...
    all_obj["OutsideCallTotalCnt"] = fd_obj["OutsideCallTotalCnt"]
